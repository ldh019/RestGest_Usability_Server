...
class ScreenManager:
    def __init__(self, screen, screen_width, screen_height, fonts):
        ...
        self.main_menu_button_rect = pygame.Rect(self.screen_width // 2 + 50, 550, 300, 50)
    ...
    def draw_back_button(self):
        ...
        text_surface = self.fonts['small'].render("Back", True, self.BLACK)
    ...
    def draw_stop_button(self):
        ...
        text_surface = self.fonts['small'].render("Stop Game", True, self.WHITE)
    ...
    def draw_user_number_input(self, user_number):
        ...
        title_text = self.fonts['large'].render("Enter user number using keyboard", True, self.BLACK)
        ...
        guide_text = self.fonts['small'].render("Press spacebar after input.", True, self.BLACK)
    ...
    def draw_condition_selection(self, condition_input):
        ...
        title_text = self.fonts['large'].render("Select condition", True, self.BLACK)
        ...
        guide_text = self.fonts['small'].render("Press spacebar after selection.", True, self.BLACK)
    ...
    def draw_device_connection(self, server_ip, server_port, device1_connected, device2_connected):
        self.screen.fill(self.WHITE)
        self.draw_back_button()
        title_text = self.fonts['large'].render("Device Connection", True, self.BLACK)
        title_rect = title_text.get_rect(center=(self.screen_width // 2, 150))
        self.screen.blit(title_text, title_rect)
        info_text = self.fonts['medium'].render(f"Server IP: {server_ip} / Port: {server_port}", True, self.BLACK)
        info_rect = info_text.get_rect(center=(self.screen_width // 2, 300))
        self.screen.blit(info_text, info_rect)
        device1_status_text = f"Device 1: {'Connected' if device1_connected else 'Not Connected'}"
        ...
        device2_status_text = f"Device 2: {'Connected' if device2_connected else 'Not Connected'}"
        ...
        if device1_connected and device2_connected:
            message_text = self.fonts['medium'].render("Both devices connected. Press spacebar to proceed.", True, self.BLUE)
            message_rect = message_text.get_rect(center=(self.screen_width // 2, 750))
            self.screen.blit(message_text, message_rect)
        else:
            message_text = self.fonts['medium'].render("Connecting devices...", True, self.BLACK)
            message_rect = message_text.get_rect(center=(self.screen_width // 2, 750))
            self.screen.blit(message_text, message_rect)
    ...
    def draw_waiting_for_data(self, device1_data_received, device2_data_received):
        self.screen.fill(self.WHITE)
        self.draw_back_button()
        title_text = self.fonts['large'].render("Waiting Before Game Start", True, self.BLACK)
        title_rect = title_text.get_rect(center=(self.screen_width // 2, 250))
        self.screen.blit(title_text, title_rect)

        status_text1 = f"Device 1: {'Data Received' if device1_data_received else 'Waiting for Data'}"
        ...
        status_text2 = f"Device 2: {'Data Received' if device2_data_received else 'Waiting for Data'}"
        ...
        if device1_data_received and device2_data_received:
            ready_text = self.fonts['medium'].render("Data received from all devices. Game will start shortly.", True, self.BLUE)
    ...
    def draw_in_progress(self, snake_list, snake_block_size, food_x, food_y):
        ...
        score_text = self.fonts['medium'].render(f"Score: {len(snake_list) - 1}", True, self.WHITE)
        score_rect = score_text.get_rect(topleft=(20, 20))
        self.screen.blit(score_text, score_rect)

    def draw_results(self, game_duration, game_score):
        self.screen.fill(self.WHITE)
        title_text = self.fonts['large'].render("Game Over", True, self.BLACK)
        title_rect = title_text.get_rect(center=(self.screen_width // 2, 150))
        self.screen.blit(title_text, title_rect)
        duration_text = self.fonts['medium'].render(f"Survival Time: {int(game_duration)} seconds", True, self.BLACK)
        duration_rect = duration_text.get_rect(center=(self.screen_width // 2, 350))
        self.screen.blit(duration_text, duration_rect)
        score_text = self.fonts['medium'].render(f"Final Score: {game_score} points", True, self.BLACK)
        ...
        restart_text = self.fonts['medium'].render("Restart", True, self.WHITE)
        ...
        main_text = self.fonts['medium'].render("Return to Main", True, self.WHITE)
